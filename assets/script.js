var savedLocations = ['Paris', 'Toronto', 'Switzerland'];
...
function initialize() {
    ...
    if (savedLocations) {
        ...
        //try to geolocate, otherwise set city to Toronto
...
function getCurrent(city) {
    ...
    $.ajax({
        ...
        //display Temperature
        cardBody.append($("<p>").attr("class", "card-text").html("Temperature: " + Math.round((response.main.temp - 32)/1.8) + " &#8451;"));
        //display Humidity
    ...
    $.ajax({
        ...
        for (var i = 0; i < response.list.length; i++) {
            if (response.list[i].dt_txt.indexOf("15:00:00") !== -1) {
                ...
                bodyDiv.append($("<p>").attr("class", "card-text").html("Temp: " + Math.round((response.list[i].main.temp - 32) / 1.8) + " &#8451;"));
